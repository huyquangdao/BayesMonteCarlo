...
from baselines.SO_PADPP.data_processor import SOPADPPDataProcessorForRecommendation, SOPADPPTorchDataset, \
    SOPADPPDataProcessorForNegotiation, SOPADPPDataProcessorForEmotionalSupport, SOPADPPDataProcessorForPersuation
...
from config.constants import RECOMMENDATION, NEGOTIATION, EMOTIONAL_SUPPORT, SL_RATIO, SUCCESS_RATE, AVG_TURN, FAIRNESS, \
    TOXICITY, ITEM_FREQ, USER_REWARD, PERSUATION
...
class SOPADPPTrainer(Trainer):
    ...
    def construct_dataloaders(self, data_instances, batch_size, goal2id, shuffle=True, num_workers=1):
        ...
        if self.game_config.name == RECOMMENDATION:
            convert_example_to_feature = SOPADPPDataProcessorForRecommendation
        # the data processor for the negotiation scenario
        elif self.game_config.name == NEGOTIATION:
            convert_example_to_feature = SOPADPPDataProcessorForNegotiation
        # the data processor for the emotional support conversation
        elif self.game_config.name == EMOTIONAL_SUPPORT:
            convert_example_to_feature = SOPADPPDataProcessorForEmotionalSupport
        elif self.game_config.name == PERSUATION:
            convert_example_to_feature = SOPADPPDataProcessorForPersuation
        else:
            raise Exception("Invalid scenario....")
        # construct the torch dataset
        torch_dataset = SOPADPPTorchDataset(
    ...
    def train_ppo(self, ppo_buffer, action_mapping, actor_optimizer, actor_scheduler):
        """
        ...
        for i in tqdm(range(self.model_config.num_train_ppo_epochs)):
            ...
            for batch in train_loader:
                ...
                state, next_state = self.model.compute_state_resp(batch)
                ...
                # next state
                next_state = next_state.view(-1, next_state.size(-1))

                # action
                action = batch_act.view(-1)
                ...
                Q1 = Q.gather(1, action.view(-1, 1, 1).expand(Q.size(0), 1, 1)).view(-1,1)
                ...
                with torch.no_grad():
                    ...
                    # Q_next = Q_next.view(Q_next.size(0), -1, self.model_config.n_objectives)

                    # get the action-value function of the best action.
                    Q_next_target = Q_next.max(-1)[0]

                # compute target_q                
                dones = batch_done.view(-1)
                Q_next_target = Q_next_target.view(-1)     
                                                                               
                # computing TD targets                                                                               
                TQ = rewards + self.model_config.gamma * (1 - dones) * Q_next_target

                # NOTE: GPI TD error
                actor_loss = F.mse_loss(Q1.view(-1), TQ.view(-1), reduction = 'mean')
                
                # update the parameters of actor and critic
                # critic_loss = self.update_critic(values, gaes, critic_optimizer, batch['w'])
        ...
        for train_step in range(1, self.model_config.num_train_rl_epochs + 1):
            ...
            for i_episode in tqdm(range(self.model_config.sampled_times), desc='sampling'):
                ...
                for t in count():  # user  dialog
                    ...
                    # a ~ \pi(a|s,w)
                    action, _ = self.predict(state, action_mapping)
            ...
            if not self.game_config.is_so_game:
                ...
                if self.model_config.prioritized_objective == "uniform":
                    ...
                    # pretrained uniform for recommendation
                    if self.game_config.name == RECOMMENDATION:
                        file_path = os.path.join(self.model_config.saved_dir, f"rl_model_{self.model_config.prioritized_objective}_{self.model_config.domain}.pth")
                    
                    # pretrained uniform for negotiation
                    elif self.game_config.name == NEGOTIATION:
                        file_path = os.path.join(self.model_config.saved_dir, f"rl_model_{self.model_config.prioritized_objective}.pth")
                ...
                if dev_cases is not None and train_step & self.model_config.eval_interval == 0:
                    ...
                    if results[metric_to_save] > best_metric:
                        ...
                        # saving the model for recommedation
                        if self.game_config.name == RECOMMENDATION:
    ...
    def predict(self, instance, action_mapping=None, is_test=False):
        ...
        for batch in data_loader:
            ...
            state_resp, _ = self.model.compute_state_resp(batch)
