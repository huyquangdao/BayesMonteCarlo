...
from utils.game import create_cases, create_target_set
from text_gen.bart_generation import BARTGeneration


class ProactivePipeline(Pipeline):
...
class ProactivePipelineForRecommendation(ProactivePipeline):
    ...
    def run_online_test(self, target_items=None, simulators=None):
        ...
        with torch.no_grad():
            ...
            # i.e using 1 process.
            action_mapping = self.dataset.construct_action_mapping(combine=False)[0]
            goals = list(action_mapping.keys())
            domain = self.dataset_config.domain
...
class ProactivePipelineForNegotiation(ProactivePipeline):
...
class ProactivePipelineForEmotionalSupport(ProactivePipeline):
...
class ProactivePipelineForPersuation(ProactivePipeline):
